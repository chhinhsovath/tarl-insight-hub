...
export async function PUT(
  ...
  try {
    ...
    const existingUserResult = await pool.query(
      "SELECT id, full_name, email, role, school_id, phone, is_active FROM tbl_tarl_users WHERE id = $1",
      [userId]
    );
    ...
    const { full_name, email, role, school_id, phone, is_active } = updatedUserData;
    ...
    const result = await pool.query(
      `UPDATE tbl_tarl_users
       SET full_name = $1,
           ...
           updated_at = NOW()
       WHERE id = $7
       RETURNING id, full_name, email, role, school_id, phone, is_active, created_at`,
      [full_name, email, role, school_id, phone, is_active, userId]
    );
    ...
    await pool.query(
      ...
      [
        currentUser.id,
        "update_user",
        `Updated user: ${updatedUserData.full_name} (${updatedUserData.email})`,
      ]
    );
