-- =====================================================
-- COMPREHENSIVE AUDIT SYSTEM AND SOFT DELETE SCHEMA
-- This ensures all user activities are recorded and no data is permanently lost
-- =====================================================
...
-- Enhanced user activities table for comprehensive logging
CREATE TABLE IF NOT EXISTS tbl_tarl_user_activities (
    id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES tbl_tarl_users(id),
    username VARCHAR(255),
    user_role VARCHAR(50),
    action_type VARCHAR(50) NOT NULL, -- CREATE, READ, UPDATE, DELETE, LOGIN, LOGOUT, RESTORE
    table_name VARCHAR(100),
    record_id INTEGER,
    old_data JSONB, -- Store the old record data before changes
    new_data JSONB, -- Store the new record data after changes
...
CREATE INDEX IF NOT EXISTS idx_user_activities_soft_delete ON tbl_tarl_user_activities (is_soft_delete);
...
CREATE TABLE IF NOT EXISTS tbl_tarl_deleted_records (
    ...
    original_data JSONB NOT NULL, -- Complete record data before deletion
    ...
    retention_period_days INTEGER DEFAULT 365, -- How long to keep deleted records
...
BEGIN
    FOREACH table_record.table_name IN ARRAY tables_to_update
    LOOP
        -- Add deleted_at column if it doesn't exist
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name = table_record.table_name AND column_name = 'deleted_at') THEN
            EXECUTE format('ALTER TABLE %I ADD COLUMN deleted_at TIMESTAMP', table_record.table_name);
        END IF;
        
        -- Add deleted_by column if it doesn't exist
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name = table_record.table_name AND column_name = 'deleted_by') THEN
            EXECUTE format('ALTER TABLE %I ADD COLUMN deleted_by INTEGER', table_record.table_name);
        END IF;
        
        -- Add is_deleted column if it doesn't exist (in addition to existing is_active)
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
BEGIN
    ...
    -- This would be set by the application before making changes
...
COMMENT ON TABLE tbl_tarl_user_activities IS 'Comprehensive audit log of all user activities in the system';
COMMENT ON TABLE tbl_tarl_deleted_records IS 'Tracks all soft-deleted records for potential restoration';
COMMENT ON FUNCTION soft_delete_record IS 'Safely soft-delete a record while maintaining audit trail';
COMMENT ON FUNCTION restore_deleted_record IS 'Restore a previously soft-deleted record';
COMMENT ON FUNCTION cleanup_old_deleted_records IS 'Permanently remove old soft-deleted records';

-- =====================================================
-- 10. SUCCESS MESSAGE
-- =====================================================

SELECT 'Comprehensive audit system and soft delete functionality has been successfully implemented!' as status;